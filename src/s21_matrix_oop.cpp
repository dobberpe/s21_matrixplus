...
void S21Matrix::edit_rows(int rows) {
  ...
  for (int i = 0; i < rows; ++i) {
    matrix[i] = (double*)calloc(cols_, sizeof(double));
    if (i < rows_) {
...
void S21Matrix::edit_cols(int cols) {
  ...
  double** matrix = (double**)malloc(rows_ * sizeof(double*));
  for (int i = 0; i < rows_; ++i) {
    matrix[i] = (double*)calloc(cols, sizeof(double));
    for (int j = 0; j < cols; ++j) matrix[i][j] = j < cols_ ? matrix_[i][j] : 0;
    free(matrix_[i]);
  }
  free(matrix_);
